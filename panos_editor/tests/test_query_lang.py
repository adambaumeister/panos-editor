...
class TestStringParser:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                "shared.address ip-netmask == 10.100.100.10 JOIN devices.device-group.post-rulebase.security.rules ON name == source",
                ["testhost_10.100.100.10"],
            ),
        ],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                "shared.address ip-netmask == 10.100.100.10 JOIN devices.device-group.post-rulebase.security.rules from == whatever ON name == source",
                ["testhost_10.100.100.10"],
            )
        ],
    )
